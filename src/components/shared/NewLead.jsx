                    ...
                    <div className="mt-4 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4">
                            <span className="text-sm text-gray-600 font-medium">
                                Page {newLeadsData?.currentPage || 1} of {newLeadsData?.totalPages || 1}
                            </span>
                            <div className="flex gap-2">
                                <Button
                                    variant="outline"
                                    size="sm"
                                    disabled={!newLeadsData?.currentPage || newLeadsData?.currentPage <= 1}
                                    onClick={() => {
                                        const newParams = new URLSearchParams(searchParams);
                                        newParams.set('page', (newLeadsData?.currentPage || 1) - 1);
                                ...
                                <Button
                                    variant="outline"
                                    size="sm"
                                    disabled={!newLeadsData?.currentPage || newLeadsData?.currentPage >= (newLeadsData?.totalPages || 1)}
                                    onClick={() => {
                                        const newParams = new URLSearchParams(searchParams);
                                        newParams.set('page', (newLeadsData?.currentPage || 1) + 1);
                                        setSearchParams(newParams);
