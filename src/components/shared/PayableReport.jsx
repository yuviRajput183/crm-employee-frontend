...
import { apiGetPayablesReport } from '@/services/reports.api';
...
const PayableReport = () => {
    ...
    const {
        ...
        queryKey: ['payables-report', filterParams],
        queryFn: async () => {
            const res = await apiGetPayablesReport(filterParams);
        ...
        <div className=' p-3 bg-white rounded shadow min-h-screen'>
            ...
            <div className=' flex justify-between items-center pb-2 border-b-2 '>
                <div className=' flex items-center gap-2'>
                    ...
                    <h1 className=' text-2xl font-bold text-gray-700'>Payable Report</h1>
                </div>
                <Button
                ...
                <Table className="min-w-[1600px]">
                    <TableHeader>
                        <TableRow className="bg-[#1e4d46] text-white hover:bg-[#1e4d46]">
                            ...
                            <TableHead className="text-white h-10 text-xs font-bold border-r border-[#2a635a]">GST Status</TableHead>
                            ...
                            <TableHead className="text-white h-10 text-xs font-bold border-r border-[#2a635a]">Billing %age</TableHead>
                            <TableHead className="text-white h-10 text-xs font-bold border-r border-[#2a635a]">Bill Amt.</TableHead>
                            <TableHead className="text-white h-10 text-xs font-bold border-r border-[#2a635a]">TDS %age</TableHead>
                            <TableHead className="text-white h-10 text-xs font-bold border-r border-[#2a635a]">TDS Amt.</TableHead>
                            <TableHead className="text-white h-10 text-xs font-bold border-r border-[#2a635a]">Net Due Amt.</TableHead>
                            <TableHead className="text-white h-10 text-xs font-bold border-r border-[#2a635a]">Paid Amt.</TableHead>
                    ...
                    <TableBody>
                        {isLoading ? (
                            <TableRow>
                                <TableCell colSpan={20} className="text-center py-10">Loading report data...</TableCell>
                            </TableRow>
                            ...
                            reportData.data.map((row, index) => (
                                ...
                                >
                                    ...
                                    <TableCell className="text-xs py-2 border-r">{row.gstStatus}</TableCell>
                                    ...
                                    <TableCell className="text-xs py-2 border-r">{row.billingPercentage}</TableCell>
                                    <TableCell className="text-xs py-2 border-r">{row.billAmt}</TableCell>
                                    <TableCell className="text-xs py-2 border-r">{row.tdsPercentage}</TableCell>
                                    <TableCell className="text-xs py-2 border-r">{row.tdsAmt}</TableCell>
                                    <TableCell className="text-xs py-2 border-r font-bold">{row.netDueAmt}</TableCell>
                                    <TableCell className="text-xs py-2 border-r text-green-700 font-bold">{row.paidAmt}</TableCell>
                                    ...
                                    <TableCell className="text-xs py-2">
                                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${row.status === 'Paid' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700 border border-red-200'
                                            }`}>
                                            {row.status}
                                        </span>
                                    </TableCell>
                            ...
                            <TableRow>
                                <TableCell colSpan={20} className="text-center py-10 text-gray-500">No payable records found.</TableCell>
                            </TableRow>
                        )}
                        {reportData?.totals && reportData?.data?.length > 0 && (
                            <TableRow className="bg-[#f39c12] hover:bg-[#e67e22] transition-colors border-b">
                                ...
                                <TableCell className="text-xs py-2 border-r text-white" colSpan={7}></TableCell>
                                ...
                                <TableCell className="text-xs py-2 border-r font-bold text-white">{reportData.totals.tdsAmt}</TableCell>
                                <TableCell className="text-xs py-2 border-r font-bold text-white">{reportData.totals.netDueAmt}</TableCell>
